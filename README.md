...
# Acceder a MySQL
docker exec -it mysql-db mysql -uroot -proot alicatados_db
```

### Volúmenes

El proyecto utiliza un volumen persistente para MySQL:
...
2. Esperar 30-60 segundos (tiempo de registro)
3. Revisar logs: `docker-compose logs eureka-server`

### Problema: Gateway devuelve 503 Service Unavailable

**Solución**:
1. Verificar que el microservicio destino esté registrado en Eureka
2. Comprobar health del servicio: `curl http://localhost:8101/actuator/health`
3. Revisar configuración de rutas en Gateway
...
2. Esperar 2-3 minutos después del inicio
...
Cada microservicio de negocio se ejecuta en **2 instancias** para alta disponibilidad:

- Containers Service: puertos 8101, 8102
- Logistics Service: puertos 8111, 8112
- Accounting Service: puertos 8121, 8122
- Users Service: puertos 8131, 8132

### Balanceo de Carga

El Gateway utiliza **Spring Cloud LoadBalancer** con algoritmo round-robin para distribuir peticiones entre instancias.

### Métricas

Accede a métricas de cada servicio:

```bash
curl http://localhost:8101/actuator/metrics
...
Este es un proyecto académico desarrollado para la asignatura de Arquitectura Orientada a Servicios.

**Autor**: Eduardo Gómez
**Universidad**: [Información académica]
...
- **Repositorio**: https://github.com/edgomeza/proyecto-aos
- **Issues**: https://github.com/edgomeza/proyecto-aos/issues
...
- Gestión de contenedores y tipos
- Contratos de alquiler
- Inspecciones post-devolución
...
1. **Service Discovery Pattern**: Registro dinámico con Eureka
2. **API Gateway Pattern**: Punto de entrada único
3. **Config Server Pattern**: Configuración centralizada
4. **Database per Service**: Esquemas independientes
5. **Circuit Breaker Pattern**: Resiliencia (preparado para Resilience4j)
6. **Load Balancing**: Distribución de carga
7. **Health Check Pattern**: Monitoreo de servicios
...
- **Flexibilidad**: Diferentes tecnologías por servicio
- **Despliegue**: CI/CD independiente por servicio
- **Equipos**: Desarrollo paralelo por equipos

### Desafíos Superados

- Configuración de comunicación entre servicios
- Sincronización de bases de datos
- Tiempo de inicio en orden correcto
...
**¡Gracias por usar el Sistema de Gestión de Alquiler de Contenedores de Alicatados Plasencia!**
