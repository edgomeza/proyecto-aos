...
REM Generate unique timestamp for test data
...
echo =========================================
echo.
echo NOTA: Para mejores resultados, ejecute con base de datos limpia
echo       o reinicie los servicios antes de la primera ejecucion.
echo.
...
echo {"containerCode":"TEST-%TIMESTAMP%","containerType":{"id":1},"status":"AVAILABLE"} > temp_containers.json
...
REM rentalNumber is auto-generated by service. Uses container id=1 (should be available on first run)
echo {"container":{"id":1},"customerId":1,"startDate":"2025-11-17","expectedEndDate":"2025-11-24","deliveryAddress":"Test Address 123"} > temp_rentals.json
...
echo {"routeCode":"RT-TEST-%TIMESTAMP%","origin":"Plasencia","destination":"Caceres","scheduleDate":"2025-11-20","status":"PLANNED"} > temp_routes_post.json
...
echo {"invoiceNumber":"INV-TEST-%TIMESTAMP%","customerId":1,"invoiceDate":"2025-11-17","subtotal":100.0,"totalAmount":121.0,"status":"PENDING"} > temp_invoices_post.json
...
echo {"username":"testuser%TIMESTAMP%","email":"test%TIMESTAMP%@example.com","password":"Test123456","role":"CUSTOMER","active":true} > temp_users_post.json
