...
public class OpenAPIConfig {
    ...
    @Bean
    public OpenAPI containerServiceOpenAPI() {
        Server localServer = new Server();
        localServer.setUrl("http://localhost:" + serverPort);
        localServer.setDescription("Servidor local");

        Server gatewayServer = new Server();
        gatewayServer.setUrl("http://localhost:8080/api/containers");
        ...
        Info info = new Info()
                .title("Containers Service API")
                .version("1.0.0")
                .description("API REST para gestión de inventario de contenedores, alquileres, tarifas e inspecciones")
                .contact(contact)
                .license(license);
